...
# These read only minimal columns from the FIRST sheet and reset the upload stream so it can be reused.
...
def _qf_read_cols_first_sheet(upload):
    ...
    _reset_upload(upload)  # allow re-reading later
...
with st.expander("🔎 Quick Query Console (instant; pick grouping)"):
    ...
    reg_filter = st.text_input("Filter Region(s) (comma-separated, optional)").strip()
    reg_filter = [r.strip() for r in reg_filter.split(",")] if reg_filter else None
    ign_filter = st.text_input("Filter ignator(s) (comma-separated, optional)").strip()
    ign_filter = [r.strip() for r in ign_filter.split(",")] if ign_filter else None
        ...
        if not q.empty:
            ...
            if "Region" in q.columns and "Completion_Rate_%" in q.columns and q["Region"].nunique() <= 50:
                fig = px.bar(q, x="Region", y="Completion_Rate_%",
                             hover_data=["DA_Pre","DA_Post","Rows"] + (["Unique_Ignators"] if "Unique_Ignators" in q.columns else []))
                fig.update_layout(xaxis_title="", yaxis_title="Completion Rate (%)")
                st.plotly_chart(fig, use_container_width=True)
