# app.py — Agastya DA (v1.6 insights + instant modes)
# Raw .xls/.xlsx/.csv -> clean -> detect Pre/Post -> score -> aggregates -> visual dashboards -> Excel
# Includes:
#   ⚡ Quick-Fire: Region + ignators (instant, minimal columns, first sheet only)
#   🔎 Quick Query Console (instant ad-hoc grouping & filters)
...
APP_VER = "AIO v1.6 (insights+) + Instant Modes"
...
# ───────────────────────── INSTANT MODES (helpers) ─────────────────────────
def _reset_upload(upload):
    try:
        upload.seek(0)
    except Exception:
        pass

def _qf_read_cols_first_sheet(upload):
    name = upload.name if hasattr(upload, "name") else "uploaded"
    suffix = name.lower().rsplit(".", 1)[-1] if "." in name else ""
    raw = upload.read()
    bio = BytesIO(raw)
    if suffix == "csv":
        cols = pd.read_csv(bio, nrows=0).columns.tolist()
        sheet = None
    else:
        try:
            xls = pd.ExcelFile(bio, engine="openpyxl" if suffix in {"xlsx","xlsm"} else None)
        ...
        bio.seek(0)
        engine = "openpyxl" if suffix in {"xlsx","xlsm"} else "xlrd"
        cols = pd.read_excel(bio, sheet_name=sheet, nrows=0, engine=engine).columns.tolist()
    _reset_upload(upload)
    return raw, sheet, cols, suffix

def _qf_detect_q_cols_from_list(cols):
    norm = {c: re.sub(r"\s+", "", str(c)).lower() for c in cols}
    pre_q, post_q = [], []
    for c, n in norm.items():
        n2 = n.replace("_","")
        if re.match(r"^q\d+$", n2):     pre_q.append(c)
        if re.match(r"^q\d+post$", n2): post_q.append(c)
    return sorted(pre_q), sorted(post_q)

def _qf_present_any(df_sub: pd.DataFrame) -> pd.Series:
    if df_sub.shape[1] == 0:
        return pd.Series(False, index=df_sub.index)
    bad = df_sub.isna() | df_sub.isin(list(MISSING_TOKENS))
    return (~bad).any(axis=1)

def _qf_read_minimal(raw_bytes, sheet_name, need_cols, suffix_hint):
    bio = BytesIO(raw_bytes)
    usecols = list(need_cols)
    if suffix_hint == "csv":
        return pd.read_csv(bio, usecols=usecols, dtype=str, low_memory=False)
    engine = "openpyxl" if suffix_hint in {"xlsx","xlsm"} else "xlrd"
    return pd.read_excel(bio, sheet_name=sheet_name, usecols=usecols, dtype=str, engine=engine)

@st.cache_data(show_spinner=False)
def quick_fire_region_ignators(upload):
    """Ultra-fast region summary with ignator counts (no scoring)."""
    raw, sheet, cols, suffix = _qf_read_cols_first_sheet(upload)
    pre_q, post_q = _qf_detect_q_cols_from_list(cols)

    need = {"Region", "Instructor Name"} | set(pre_q) | set(post_q)
    if "Date" in cols: need.add("Date")
    if "Date_Post" in cols: need.add("Date_Post")

    df = _qf_read_minimal(raw, sheet, need, suffix)
    if "Region" not in df.columns: df["Region"] = "Unknown"
    if "Instructor Name" not in df.columns: df["Instructor Name"] = "Unknown"

    pre_cols  = [c for c in pre_q if c in df.columns]  + (["Date"] if "Date" in df.columns else [])
    post_cols = [c for c in post_q if c in df.columns] + (["Date_Post"] if "Date_Post" in df.columns else [])

    df["DA_Pre_Done"]  = _qf_present_any(df[pre_cols])  if pre_cols  else False
    df["DA_Post_Done"] = _qf_present_any(df[post_cols]) if post_cols else False

    reg = (df.groupby("Region", dropna=False, observed=True)
             .agg(Rows=("Region","size"),
                  Unique_Ignators=("Instructor Name", pd.Series.nunique),
                  DA_Pre=("DA_Pre_Done","sum"),
                  DA_Post=("DA_Post_Done","sum")).reset_index())

    per_ign = (df.groupby(["Region","Instructor Name"], observed=True)
                 .agg(Pre_Done=("DA_Pre_Done","any"),
                      Post_Done=("DA_Post_Done","any")).reset_index())
    ign_pre  = per_ign.groupby("Region", observed=True)["Pre_Done"].sum().rename("Ignators_with_Pre")
    ign_post = per_ign.groupby("Region", observed=True)["Post_Done"].sum().rename("Ignators_with_Post")

    reg = reg.merge(ign_pre, on="Region", how="left").merge(ign_post, on="Region", how="left")
    for c in ["Ignators_with_Pre","Ignators_with_Post"]:
        if c not in reg.columns: reg[c] = 0

    reg["Completion_Rate_%"]   = np.where(reg["DA_Pre"]>0, (reg["DA_Post"]/reg["DA_Pre"])*100, np.nan)
    reg["Pre_per_Ignator"]     = np.where(reg["Unique_Ignators"]>0, reg["DA_Pre"]/reg["Unique_Ignators"], np.nan)
    reg["Post_per_Ignator"]    = np.where(reg["Unique_Ignators"]>0, reg["DA_Post"]/reg["Unique_Ignators"], np.nan)
    reg["Ignator_Post_Rate_%"] = np.where(reg["Ignators_with_Pre"]>0, (reg["Ignators_with_Post"]/reg["Ignators_with_Pre"])*100, np.nan)
    return reg.sort_values(["Completion_Rate_%","DA_Pre"], ascending=[False, False])

@st.cache_data(show_spinner=False)
def quick_query_table(upload, dims, want_region_filter=None, want_ign_filter=None):
    """Instant ad-hoc grouping without full processing."""
    raw, sheet, cols, suffix = _qf_read_cols_first_sheet(upload)
    pre_q, post_q = _qf_detect_q_cols_from_list(cols)

    need = set(dims) | set(pre_q) | set(post_q)
    if "Date" in cols: need.add("Date")
    if "Date_Post" in cols: need.add("Date_Post")
    if want_region_filter: need.add("Region")
    if want_ign_filter:    need.add("Instructor Name")

    df = _qf_read_minimal(raw, sheet, need, suffix)
    if want_region_filter and "Region" in df.columns:
        df = df[df["Region"].astype(str).isin(want_region_filter)]
    if want_ign_filter and "Instructor Name" in df.columns:
        df = df[df["Instructor Name"].astype(str).isin(want_ign_filter)]
    ...
    grp = (df.groupby(list(dims), dropna=False, observed=True)
             .agg(Rows=(list(dims)[0], "size"),
                  DA_Pre=("DA_Pre_Done","sum"),
                  DA_Post=("DA_Post_Done","sum")).reset_index())

    if "Instructor Name" not in dims and "Instructor Name" in df.columns:
        igns = df.groupby(list(dims), observed=True)["Instructor Name"].nunique().rename("Unique_Ignators")
        grp = grp.merge(igns, on=list(dims), how="left")

    grp["Completion_Rate_%"] = np.where(grp["DA_Pre"]>0, (grp["DA_Post"]/grp["DA_Pre"])*100, np.nan)
    return grp
...
# ───────────────────────── Unique assessment sessions ─────────────────────────
def build_assessment_sessions(df: pd.DataFrame):
    """
    Returns:
      sessions: unique rows (ignator × date × time × topic × school × phase)
      summary: per-ignator counts: Unique_Pre_Sessions, Unique_Post_Sessions, Unique_Sessions_Overall
    """
    if df is None or df.empty:
        return pd.DataFrame(), pd.DataFrame()

    d = df.copy()
    for c in ["Instructor Name", "School Name", "Topic Name", "Date", "Time", "Date_Post", "Time Post",
              "DA_Pre_Done", "DA_Post_Done"]:
        if c not in d.columns:
            if c in {"Date", "Date_Post"}:
                d[c] = pd.NaT
            elif c in {"Time", "Time Post"}:
                d[c] = pd.NaT
            elif c in {"DA_Pre_Done", "DA_Post_Done"}:
                d[c] = False
            else:
                d[c] = np.nan
    ...
    d["Date_Post"] = pd.to_datetime(d["Date_Post"], errors="coerce")

    # PRE sessions
    pre = d[d["DA_Pre_Done"] == True].copy()
    pre = pre.dropna(subset=["Instructor Name", "School Name", "Topic Name"])
    pre = pre.rename(columns={"Date": "Session_Date", "Time": "Session_Time"})
    pre = pre[["Instructor Name", "School Name", "Topic Name", "Session_Date", "Session_Time"]]
    pre = pre.dropna(subset=["Session_Date"])
    pre["Phase"] = "Pre"

    # POST sessions
    post = d[d["DA_Post_Done"] == True].copy()
    post = post.dropna(subset=["Instructor Name", "School Name", "Topic Name"])
    post = post.rename(columns={"Date_Post": "Session_Date", "Time Post": "Session_Time"})
    post = post[["Instructor Name", "School Name", "Topic Name", "Session_Date", "Session_Time"]]
    post = post.dropna(subset=["Session_Date"])
    post["Phase"] = "Post"
    ...
    # Unique per ignator/date/time/topic/school/phase
    sessions = both.drop_duplicates(
        subset=["Instructor Name", "School Name", "Topic Name", "Session_Date", "Session_Time", "Phase"]
    )

    # Overall union (phase-agnostic)
    overall = sessions.drop_duplicates(
        subset=["Instructor Name", "School Name", "Topic Name", "Session_Date", "Session_Time"]
    )

    # Counts per ignator by phase
    by_ign_phase = (
        sessions.groupby(["Instructor Name", "Phase"], observed=True)
        .size()
        .reset_index(name="Unique_Sessions")
        .pivot(index="Instructor Name", columns="Phase", values="Unique_Sessions")
        .reset_index()
    )
    # Ensure expected columns exist
    if "Pre" not in by_ign_phase.columns:  by_ign_phase["Pre"] = 0
    if "Post" not in by_ign_phase.columns: by_ign_phase["Post"] = 0
    by_ign_phase = by_ign_phase.rename(columns={"Pre": "Unique_Pre_Sessions", "Post": "Unique_Post_Sessions"})

    # Overall counts per ignator
    overall_counts = (
        overall.groupby("Instructor Name", observed=True)
        .size()
        .reset_index(name="Unique_Sessions_Overall")
    )

    # Merge WITHOUT blanket fillna on categorical cols
    summary = by_ign_phase.merge(overall_counts, on="Instructor Name", how="outer")

    # Only fill numeric session columns, then cast to int
    num_cols = ["Unique_Pre_Sessions", "Unique_Post_Sessions", "Unique_Sessions_Overall"]
    for c in num_cols:
        if c in summary.columns:
            summary[c] = pd.to_numeric(summary[c], errors="coerce").fillna(0).astype(int)

    # Sort nicely
    sort_cols = [c for c in ["Unique_Sessions_Overall", "Unique_Post_Sessions", "Unique_Pre_Sessions"] if c in summary.columns]
    if sort_cols:
...
def build_subject_topic(df):
    ...
    sub = topic = pd.DataFrame()
    if s_cols:
        sub = (row.groupby(s_cols, observed=True)
                 .agg(Rows=("Student Name","count"), DA_Pre=("DA_Pre_Done","sum"), DA_Post=("DA_Post_Done","sum"),
                      Mean_PreScore=("Pre_Score","mean"), Mean_PostScore=("Post_Score","mean"),
                      Mean_Gain=("Gain","mean"), Mean_PrePct=("Pre_%","mean"), Mean_PostPct=("Post_%","mean")).reset_index())
...
def funnel_table(df, by_cols):
    ...
    agg = (d.groupby(by_cols, observed=True)
             .agg(Assigned=("Assigned","sum"),
                  Pre_Attempted=("DA_Pre_Done","sum"),
                  Post_Completed=("DA_Post_Done","sum")).reset_index())
...
def equity_gaps(df, gap_dim="Gender"):
    ...
    if "Student Id" not in df.columns: return pd.DataFrame()
    stu = (df.groupby(["Student Id", gap_dim], observed=True)
             .agg(Pre=("DA_Pre_Done","sum"), Post=("DA_Post_Done","sum"), Mean_Gain=("Gain","mean")).reset_index())
    ...
    g = (stu.groupby(gap_dim, observed=True)
            .agg(Students=("Student Id","nunique"),
                 ...
                 Avg_Gain=("Mean_Gain","mean")).reset_index())
...
def outlier_scan(df):
    ...
    for col in ["Gain","Norm_Gain"]:
        if col in d.columns:
            std = d[col].std(ddof=0)
            z = (d[col] - d[col].mean())/ (std if std else 1)
            out[f"{col}_z"] = z
    ...
    if out:
        ...
        keep_cols = [c for c in ["Region","Instructor Name","School Name","Student Id","Student Name","Date"] if c in d.columns]
        res = pd.concat([d[keep_cols], res], axis=1)
...
# Performance toggles (for full pipeline)
...
# ───────────────────────── INSTANT MODES (before heavy read) ─────────────────────────
with st.expander("⚡ Quick-Fire: Region + ignators (instant; minimal columns)"):
    if st.button("Run Quick-Fire"):
        with st.spinner("Crunching region + ignator numbers…"):
            qf = quick_fire_region_ignators(uploaded)
        if not qf.empty:
            st.dataframe(qf, use_container_width=True, hide_index=True)
            cA, cB = st.columns(2)
            with cA:
                fig = px.bar(qf, x="Region", y="Completion_Rate_%",
                             hover_data=["DA_Pre","DA_Post","Rows","Unique_Ignators","Ignators_with_Pre","Ignators_with_Post"])
                fig.update_layout(xaxis_title="", yaxis_title="Completion Rate (%)")
                st.plotly_chart(fig, use_container_width=True)
            with cB:
                fig2 = px.bar(qf, x="Region", y="Ignator_Post_Rate_%")
                fig2.update_layout(xaxis_title="", yaxis_title="Ignator Post Rate (%)")
                st.plotly_chart(fig2, use_container_width=True)
            st.download_button("Download Quick-Fire (CSV)",
                               qf.to_csv(index=False).encode("utf-8"),
                               "quick_fire_region_ignators.csv", "text/csv")
        else:
            st.info("No rows found.")

with st.expander("🔎 Quick Query Console (instant; pick grouping)"):
    preset = st.selectbox("Group by…", ["Region", "Region + Instructor", "Instructor", "Subject", "State", "School Name"])
    dims = {"Region":["Region"],
            "Region + Instructor":["Region","Instructor Name"],
            "Instructor":["Instructor Name"],
            "Subject":["Subject"],
            "State":["State"],
            "School Name":["School Name"]}[preset]

    reg_filter_in = st.text_input("Filter Region(s) (comma-separated, optional)")
    ign_filter_in = st.text_input("Filter ignator(s) (comma-separated, optional)")
    reg_filter = [r.strip() for r in reg_filter_in.split(",") if r.strip()] if reg_filter_in else None
    ign_filter = [r.strip() for r in ign_filter_in.split(",") if r.strip()] if ign_filter_in else None

    if st.button("Run Quick Query"):
        with st.spinner("Running quick query…"):
            q = quick_query_table(uploaded, tuple(dims), want_region_filter=reg_filter, want_ign_filter=ign_filter)
        if not q.empty:
            q = q.sort_values(["Completion_Rate_%","DA_Pre"], ascending=[False, False])
            st.dataframe(q, use_container_width=True, hide_index=True)
            if "Region" in q.columns and "Completion_Rate_%" in q.columns and q["Region"].nunique() <= 50:
                fig = px.bar(q, x="Region", y="Completion_Rate_%",
                             hover_data=[c for c in ["DA_Pre","DA_Post","Rows","Unique_Ignators"] if c in q.columns])
                fig.update_layout(xaxis_title="", yaxis_title="Completion Rate (%)")
                st.plotly_chart(fig, use_container_width=True)
            st.download_button("Download Quick Query (CSV)", q.to_csv(index=False).encode("utf-8"),
                               "quick_query.csv", "text/csv")
        else:
            st.info("No rows matched.")

st.divider()

# ───────────────────────── Now read the first sheet (for full pipeline) ─────────────────────────
try:
...
# Data Quality (light)
diag = {}
for c in meta.get("pre_q_cols", []) + meta.get("post_q_cols", []):
...
anomalies = pd.DataFrame() if fast_summary else df_clean[
    (df_clean.get("Anom_PostBeforePre", False)) | (df_clean.get("Anom_SameTimestamp", False))
].copy()
...
with st.expander("🧮 Unique assessment sessions by ignator"):
    # Respect current filters:
    sess_tbl, sess_summary = build_assessment_sessions(df_clean_v)

    if not sess_summary.empty:
        st.subheader("Counts per ignator")
        st.dataframe(sess_summary, use_container_width=True, hide_index=True)
        st.download_button(
            "Download session counts (CSV)",
            sess_summary.to_csv(index=False).encode("utf-8"),
            "ignator_unique_sessions.csv",
            "text/csv"
        )
    else:
        st.info("No sessions found under current filters.")

    st.subheader("All unique sessions (detailed)")
    if not sess_tbl.empty:
        st.dataframe(
            sess_tbl.sort_values(["Instructor Name", "Session_Date", "Session_Time", "School Name", "Topic Name"]),
            use_container_width=True, hide_index=True
        )
        st.download_button(
            "Download detailed sessions (CSV)",
            sess_tbl.to_csv(index=False).encode("utf-8"),
            "ignator_sessions_detailed.csv",
            "text/csv"
        )
    else:
        st.info("No detailed sessions available.")
...
# ── Subject / Topic ──
with tab6:
    subj_v, topic_v = build_subject_topic(df_clean_v)
    cA, cB = st.columns(2)
    with cA:
        st.subheader("Subject summary")
        if not subj_v.empty:
            d = subj_v.sort_values("Completion_Rate_%", ascending=False)
    ...
    with cB:
        ...
        if not topic_v.empty:
            d = topic_v.sort_values(["Subject","Completion_Rate_%"], ascending=[True, False])
            st.dataframe(d, use_container_width=True, hide_index=True)
...
with tab7:
    ...
    if "School Name" in df_clean_v.columns:
        ...
        st.dataframe(schools.sort_values(["Completion_Rate_%","DA_Pre"], ascending=[False, False]),
                     use_container_width=True, hide_index=True)
        st.download_button("Download School Summary (CSV)", schools.to_csv(index=False).encode("utf-8"),
                           "school_summary.csv", "text/csv")
    if "State" in df_clean_v.columns:
        ...
        st.dataframe(states.sort_values(["Completion_Rate_%","DA_Pre"], ascending=[False, False]),
                     use_container_width=True, hide_index=True)
        st.download_button("Download State Summary (CSV)", states.to_csv(index=False).encode("utf-8"),
...
with tab8:
    ...
    if not gen_gaps.empty:
        ...
        with cD:
            if "Gender" in stu.columns and not stu["Mean_Gain"].dropna().empty:
