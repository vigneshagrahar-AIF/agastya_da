# app.py — All-in-one DA pipeline + dashboard (fast, robust, first-sheet-only)
# Raw .xls/.xlsx/.csv -> clean -> detect Pre/Post -> score -> aggregate -> dashboard -> export Excel
...
APP_VER = "AIO v1.4 (first-sheet only + fast)"
...
def read_raw_first_sheet(upload) -> tuple[pd.DataFrame, dict]:
    ...
    try:
        if suffix == "csv":
            ...
            # read bytes so we can open twice (to get sheet names reliably)
            raw = upload.read()
            bio = BytesIO(raw)
...
# ========================= UI =========================
st.markdown(f"### Agastya – DA (All-in-One)  \n*{APP_VER}*  \n*(First sheet only is processed for Excel files)*")

# Performance toggles
c1, c2, c3 = st.columns(3)
with c1:
    fast_summary = st.toggle("Fast Summary Mode (skip per-question & heavy tables)", value=False)
with c2:
    show_sample = st.toggle("Show a 200-row sample preview", value=False)
with c3:
    exclude_dupes = st.toggle("Exclude potential duplicates (Student Id + Topic + Date)", value=True)

uploaded = st.file_uploader("Upload raw data (.xls / .xlsx / .csv)", type=["xls","xlsx","csv"])
if not uploaded:
    st.info("Upload your file. The app will read only the FIRST sheet if Excel.")
    st.stop()
...
if exclude_dupes and {"Student Id","Topic Name","Date"}.issubset(df_clean.columns):
    ...
    if mask_dup.any():
        df_nd = df_clean.loc[~mask_dup].copy()
        # quick re-aggregate
        reg = (df_nd.groupby("Region", dropna=False, observed=True)
                 .agg(DA_Pre=("DA_Pre_Done","sum"), DA_Post=("DA_Post_Done","sum"),
                      ...
                      Mean_Norm_Gain=("Norm_Gain","mean"))
                 .reset_index()); add_completion_rate(reg)
        ign = (df_nd.groupby("Instructor Name", dropna=False, observed=True)
                 .agg(DA_Pre=("DA_Pre_Done","sum"), DA_Post=("DA_Post_Done","sum"),
                      Rows=("Student Name","count"),
                      Regions=("Region", pd.Series.nunique),
                      Schools=("School Name", pd.Series.nunique),
                      ...
                      Mean_Gain=("Gain","mean"), Median_Gain=("Gain","median"),
                      Mean_Norm_Gain=("Norm_Gain","mean"))
                 .reset_index()); add_completion_rate(ign)
        ri = (df_nd.groupby(["Region","Instructor Name"], dropna=False, observed=True)
                 .agg(DA_Pre=("DA_Pre_Done","sum"), DA_Post=("DA_Post_Done","sum"),
                      Rows=("Student Name","count"),
                      Schools=("School Name", pd.Series.nunique),
                      Mean_Pre=("Pre_Score","mean"), Mean_Post=("Post_Score","mean"),
                      Mean_Gain=("Gain","mean"), Median_Gain=("Gain","median"),
                      Mean_Norm_Gain=("Norm_Gain","mean"))
                 .reset_index()); add_completion_rate(ri)
        reg_ign_status = (
            df_nd.groupby(["Region","Instructor Name"], observed=True)
                 .agg(Pre_Done=("DA_Pre_Done","any"),
                      Post_Done=("DA_Post_Done","any"))
                 .reset_index()
        )
